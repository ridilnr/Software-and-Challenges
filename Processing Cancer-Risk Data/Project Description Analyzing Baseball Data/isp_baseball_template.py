...
def filter_by_year(statistics, year, yearid):
    ...
    """
    new_list = list()
    for num in range(len(statistics)):
        if (yearid, str(year)) in statistics[num].items():
            new_list.append(statistics[num])
    return new_list
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    new_list = list()
    for num in range(len(statistics)):
        new_list.append((statistics[num][info['playerid']], formula(info, statistics[num])))
    new_list.sort(reverse=True, key=lambda pair: pair[1])
    top_players = list()
    for num in range(numplayers):
        top_players.append(new_list[num])
    return top_players
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    key_field = info['playerid']
    separator = info['separator']
    quote = info['quote']
    first_name = info['firstname']
    last_name = info['lastname']
    my_list = read_csv_as_nested_dict(filename, key_field, separator, quote)
    player_names = list()
    for idx in range(len(top_ids_and_stats)):
        player_id = top_ids_and_stats[idx][0]
        batting = top_ids_and_stats[idx][1]
        if player_id in my_list:
            names = (my_list[player_id][first_name], my_list[player_id][last_name])
            player_names.append("{0:0.3f} --- {1} {2}".format(batting, names[0], names[1]))
    return player_names
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    """
    my_list = read_csv_as_list_dict(info['battingfile'], info['separator'], info['quote'])
    statistics = list()
    for idx in range(len(my_list)):
        my_dict = dict(my_list[idx])
        if (info['yearid'], str(year)) in my_dict.items():
            statistics.append(my_dict)
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for row in range(len(statistics)):
        key = statistics[row][playerid]
        list_players = dict()
        summ = 0
        if key not in player_id_list:
            player_id_list.append(key)
            for idx2 in range(len(fields)):
                str_stat = fields[idx2]
                list_players[playerid] = key
                str_stat_int = int(statistics[row][str_stat])
                list_players[str_stat] = summ + str_stat_int
            dict_player[key] = dict(list_players)
        else:
            for idx2 in range(len(fields)):
                str_stat = fields[idx2]
                str_stat_int = int(statistics[row][str_stat])
                dict_player[key][str_stat] = dict_player[key][str_stat] + str_stat_int
    return dict_player
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    for idx in range(len(my_list)):
        my_dict = dict(my_list[idx])
        statistics.append(my_dict)
    dict_aggregate = aggregate_by_player_id(statistics, info['playerid'], info['battingfields'])
    new_statistics = list()
    for value in dict_aggregate.values():
        new_statistics.append(value)
    list_top_players = top_player_ids(info, new_statistics, formula, numplayers)
    return lookup_player_names(info, list_top_players)
